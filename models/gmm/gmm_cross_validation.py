...
#     gmm = GMM(preprocess="z_score/" if zscore else "raw/"+"pca" if pca is not None else "raw")
#     cross_validation_for_gmm(d, l, gmm, zscore, pca)
#
# def cv_gmm_Tied(d, l, zscore=False, pca=None):
#     gmm_tied = GMMTied(preprocess="z_score/" if zscore else "raw/"+"pca" if pca is not None else "raw")
#     cross_validation_for_gmm(d, l, gmm_tied, zscore, pca)
#
# def cv_gmm_Diag(d, l, zscore=False, pca=None):
#     gmm_diag = GMMDiag(preprocess="z_score/" if zscore else "raw/"+"pca" if pca is not None else "raw")
#     cross_validation_for_gmm(d, l, gmm_diag, zscore, pca)
#
# def cv_gmm_TiedDiag(d, l, zscore=False, pca=None):
#     gmm_tied_diag = GMMTiedDiag(preprocess="z_score/" if zscore else "raw/"+"pca" if pca is not None else "raw")
